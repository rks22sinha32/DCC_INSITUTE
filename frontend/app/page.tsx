...
export default function Home() {
  ...
  // 👇 YAHAN MAGIC HAI: Mobile menu kholne/band karne ko track karne ke liye state
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        // Aapka AWS Server IP
        const response = await fetch('http://3.39.230.105:8000/api/courses/');
        if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-slate-100 font-sans selection:bg-blue-500 selection:text-slate-900">
      ...
      {/* ── PREMIUM NAVBAR ── */}
      {/* px-4 md:px-8 kiya gaya hai taaki mobile par padding sahi rahe */}
      <nav className="flex justify-between items-center px-4 md:px-8 py-4 border-b border-slate-800 bg-[#0B1120]/90 sticky top-0 z-50 backdrop-blur-md">
        ...
        {/* Enroll Button & Mobile Hamburger Icon */}
        <div className="flex items-center gap-4">
          {/* Enroll button choti screen par hide rahega taaki menu clean dikhe */}
          ...
          >
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              {isMobileMenuOpen ? (
                // Jab menu khula ho toh Cross (X) icon
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              ) : (
                // Jab menu band ho toh Hamburger (☰) icon
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="space-y-4">
              {/* Batch 1 */}
              <div className="bg-[#0B1120] border border-slate-800 rounded-xl p-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-blue-500/50 transition cursor-pointer">
                <div>
                ...
                <span className="px-3 py-1 rounded-full border border-blue-500/30 text-blue-400 text-xs font-bold bg-blue-500/10 text-center w-max">
                  RUNNING
                </span>
              </div>
              ...
              <div className="bg-[#0B1120] border border-slate-800 rounded-xl p-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-blue-500/50 transition cursor-pointer">
                ...
                <span className="px-3 py-1 rounded-full border border-orange-500/30 text-orange-400 text-xs font-bold bg-orange-500/10 text-center w-max">
                  UPCOMING
                </span>
              </div>
            </div>
